...
class Worker:
    ...
    def train(self) -> int:
        ...
        for epoch in range(
            self.exp.policy.epoch_start,
            self.exp.policy.epoch + self.exp.policy.epoch_start,
        ):
            ...
            for step, batch in enumerate(tqdm(loader, disable=self.disable_tqdm)):
                ...
                if self.exp.policy.check_interval:
                    if self.exp.policy.check_interval < 0:  # step part
                        if (step + 1) % max(
                            train_steps // (-self.exp.policy.check_interval), 1
                        ) == 0:
    ...
    def mind_large_evaluate(self, loader):
        ...
        score_series, col_series = self.base_evaluate(
            loader, cols=[item_col, group_col]
        )
    ...
    def evaluate(self, loader, metrics):
        ...
        label_col, group_col = (
            self.controller.column_map.label_col,
            self.controller.column_map.group_col,
        )
        score_series, col_series = self.base_evaluate(
            loader, cols=[label_col, group_col]
        )
    ...
    def train_runner(self):
        ...
        self.scheduler = get_linear_schedule_with_warmup(
            ...
            num_training_steps=len(self.controller.sets.train_set)
